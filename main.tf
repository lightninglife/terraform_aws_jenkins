 ...
 connection {
  ...
  provisioner "remote-exec" {
      inline = [
        ...
        "sudo git clone https://github.com/lightninglife/terraform_aws_jenkins.git /var/www/html/"
     ]
  }
}
...
resource "aws_s3_bucket" "task_bucket" {
  ...
  bucket = "terraform-ci-cd-pipeline-iac"
  acl    = "public-read"
  force_destroy = "true"
  website{
  ...
  tags = {
    Name = "terraform-ci-cd-pipeline-iac"
  }
}

resource "aws_codepipeline" "task_codepipeline" {
   name = "task_codepipeline"
   role_arn = "arn:aws:iam::464392538707:role/-codepipeline-role"
   artifact_store {
...
owner = "ThirdParty"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        BucketName = "terraform-ci-cd-pipeline-iac"
        Extract = "true"
      }
    }
  }
...
resource "null_resource" "codepipeline_cloudfront" {
  ...
  provisioner "local-exec" {
    command = "/usr/local/bin/aws s3api put-object-acl  --bucket terraform-ci-cd-pipeline-iac --acl public-read --key freddie_mercury.jpg"
  }
}
